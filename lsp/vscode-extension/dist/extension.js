...
function runCli(cmd, cwd) {
    return new Promise((resolve) => {
        (0, child_process_1.exec)(cmd, { cwd, windowsHide: true }, (error, stdout, stderr) => {
...
function parseDiagnostics(output) {
    ...
    for (const line of lines) {
        ...
        if (structuredMatch) {
            ...
            diags.push(new vscode.Diagnostic(new vscode.Range(lineNo, colNo, lineNo, colNo + 50), `${code}: ${message}`, severity));
            continue;
        }
        ...
        if (simpleMatch) {
            ...
            diags.push(new vscode.Diagnostic(new vscode.Range(lineNum, 0, lineNum, 120), message, vscode.DiagnosticSeverity.Error));
        }
    }
    return diags;
}
function activate(context) {
    ...
    const resolveCli = () => cfg().get('cdl.cliPath', 'cdl');
    ...
    async function lint(doc) {
        ...
        if (!document || document.languageId !== 'cdl')
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cdl.generateOpenAPI', async () => {
        ...
        const cli = resolveCli();
        const cfgOut = cfg().get('cdl.openapiOutput', 'out/api.openapi.yaml');
    ...
    vscode.workspace.onDidSaveTextDocument((doc) => {
        if (cfg().get('cdl.enableOnSave', true) && doc.languageId === 'cdl') {
            lint(doc);
        }
    });
